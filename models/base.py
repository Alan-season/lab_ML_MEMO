...
class BaseLearner(object):
    ...
    def samples_per_class(self):
        if self._fixed_memory:  # 如果限定内存大小
            return self._memory_per_class   # 返回内存中允许每个类存储的样例数
        else:
            assert self._total_classes != 0, "Total classes is 0"
            return self._memory_size // self._total_classes # 返回 总内存样例预算 // 总类数 = 每个类的平均样例内存预算
