...
def train(args):
    """提取seed和device，在相同的配置下用不同的seed和device进行多次训练"""
    seed_list = copy.deepcopy(args["seed"])
    device = copy.deepcopy(args["device"])

    # 遍历种子列表，为每个种子和设备信息设置参数并调用_train函数，默认只有一个seed: 1993
...
def _train(args):
    """执行具体的训练过程"""
    # 将运行时间和参数格式化成配置config和日志log，并保存到指定目录下
    ...
    # 载入模型，只有MEMO可以载入，在这一步中完成了MEMO网络通用块和专用块列表的组建
    model = factory.get_model(args["model_name"], args) # -> factory.py
    ...
    for task in range(data_manager.nb_tasks):
        ...
        # 开始增量学习
        model.incremental_train(data_manager)   # -> memo.py
